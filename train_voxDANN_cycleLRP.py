...
from func.loss_func1 import dice_accuracy, dice_loss_II, dice_loss_II_weights, dice_loss_org_weights, \
...
# File formats
train_file_format1 = '.h5'  # Source: ATAS
train_file_format2 = '.h5'  # Target: LRP (Usually .h5)
...
# Assuming standard path structure for LRP
lrp_dataset_path = '/data1/myt/dataset/LateralRootPrimordia_processed_wide_boundary/'
...
# 4.2 Load LRP (Target Domain) - Merge train, val, test
try:
    lrp_info = load_obj("dataset_info/LRP_dataset_info")
except FileNotFoundError:
    print("Error: dataset_info/LRP_dataset_info.pkl not found.")
    raise

lrp_paths = []

# LRP might be small, so we gather all available data (train/val/test) for adaptation
for split in ['train', 'val', 'test']:
    if split in lrp_info:
        for f in lrp_info[split]:
            # Check if LRP has subfolders or flat structure. Assuming flat or split folders.
            # Using os.path.join with split to be safe, if files are in root, this might need adjustment.
            # Usually LRP processed data is flat or follows standard structure.
            # We assume subfolders 'train', 'val' exist if keys exist.
            lrp_paths.append(os.path.join(lrp_dataset_path, split, f))

# Fallback if splits are missing or structure is flat (common in small datasets)
if not lrp_paths:
    print("Warning: LRP splits not found in specific folders, trying 'train' list in root...")
    for f in lrp_info.get('train', []):
        lrp_paths.append(os.path.join(lrp_dataset_path, f))

# 4.3 [Key Modification] Cycle / Oversampling Logic for LRP
num_atas = len(atas_dict)
num_lrp = len(lrp_paths)

print("Original Data -> ATAS: {}, LRP: {}".format(num_atas, num_lrp))

if num_lrp > 0 and num_atas > num_lrp:
    # Calculate repetition factor
    repeat_times = num_atas // num_lrp
    if num_atas % num_lrp != 0:
        repeat_times += 1

    print("Executing Cycle (Oversampling) on LRP... Repeating {} times".format(repeat_times))
    lrp_paths = lrp_paths * repeat_times

# Generate LRP dict with unique keys
lrp_dict = {}
for i, p in enumerate(lrp_paths):
    lrp_dict['lrp_{}'.format(i)] = p

print("Final Data (After Cycle) -> ATAS: {}, LRP: {}".format(len(atas_dict), len(lrp_dict)))

# Initialize Dataset
dataset = DomainGENDataset(data_dict1=atas_dict, data_dict2=lrp_dict)
...
for epoch in range(max_epoch):
    ...
    for i, batch in enumerate(train_loader):
        ...
        # Prepare Segmentation Ground Truth (Valid for Source/ATAS)
    ...
    if avg_seg_loss < minimum:
        ...
        save_model_path = os.path.join(save_path, "advonly_cycleLRP.pkl")
